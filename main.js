...
window.onload = function () {
  ...
  // load water normals (LOCAL)
  const waterNormals = new THREE.TextureLoader().load('./textures/waternormals.jpg', (tex) => {
    tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
  });

  // terrain painter (sand + water + others)
  const painter = new TerrainPainter(scene, dirLight, waterNormals);

  // world regen
  function regenerateWorld(width, height) {
    gridWidth = width;
    gridHeight = height;
    painter.setGridSize(width, height);
  ...
  canvas.addEventListener('pointerup', (e) => {
    ...
    if (paintingMode && currentPaintType) { painter.paintTile(tx, tz, currentPaintType); return; }
  ...
  viewport.onBeforeRender = (dt) => {
    ...
    if (delta.lengthSq() > 0) { camera.position.add(delta); controls.target.add(delta); }
    painter.tickWater(dt);
    controls.update();
  };
  ...
  function applyProjectData(data) {
    ...
    // terrain
    painter.deserialize(data.terrain?.tiles);
  ...
  function addFreezeToggle() {
    ...
    style.textContent = `
      .hud-freeze{position:fixed;top:12px;left:12px;z-index:20;display:flex;align-items:center;gap:8px;background:rgba(30,32,37,.85);color:#e8e8ea;padding:8px 10px;border:1px solid rgba(255,255,255,.1);border-radius:6px;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);font:600 12px/1.2 -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Inter,sans-serif}
    ...
    freezeCheckboxEl.addEventListener('change', () => {
      if (markerMode || paintingMode) { freezeCheckboxEl.checked = true; return; }
