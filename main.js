...
window.onload = function () {
  ...
  const paintedTileData = new Map(); // "x,y" -> type
  ...
  function regenerateWorld(width, height) {
    ...
    const terrainMat = new THREE.MeshStandardMaterial({
      vertexColors: true,
      flatShading: false,
      side: THREE.DoubleSide // ensure raycaster hits regardless of camera angle
    });
    ...
    if (!heightTool) {
      heightTool = new HeightTool(scene, terrainMesh, gridWidth, gridHeight);
    } else {
      heightTool.reset(terrainMesh, gridWidth, gridHeight);
    }

    // Pathfinding bounds == grid size
    controller.updateGridSize(gridWidth, gridHeight);

    // Center character and camera on middle tile
  ...
  uiPanel.panelElement.addEventListener('terrain-select', (e) => {
    ...
    if (active) {
      ...
      if (heightMode) { heightMode = false; pinMode = false; heightTool?.removeAllPins(); }
      currentPaintType = type;
      paintingMode = true;
      setFreeze(true, true);
    } else {
      paintingMode = false; currentPaintType = null; setFreeze(false, false);
    }
  });
  ...
  uiPanel.panelElement.addEventListener('save-project', (e) => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = filename || 'titanmap.json';
  ...
  canvas.addEventListener('pointerdown', (e) => {
    ...
    downTime = e.timeStamp || performance.now();
  }, { passive: true });
  ...
  canvas.addEventListener('pointerup', (e) => {
    ...
    const hits = raycaster.intersectObjects(collectPickables(), true);
    if (!hits.length) return;

    const p = hits[0].point;
    const { tx, tz } = worldToTile(p, gridWidth, gridHeight);
    ...
    if (!freezeTapToMove) controller.moveTo(tx, tz);
  }, { passive: true });
  ...
  function createWaterTile(tx, tz) {
    ...
    if (water.material.uniforms.size) water.material.uniforms.size.value = 10.0;
  ...
  function paintTile(tx, tz, type) {
    ...
    if (type === 'water') {
      ...
      const v_br = (tz + 1) * (wSeg + 1) + (tx + 1);

      colors.setXYZ(v_tl, color.r, color.g, color.b);
      colors.setXYZ(v_tr, color.r, color.g, color.b);
      colors.setXYZ(v_bl, color.r, color.g, color.b);
      colors.setXYZ(v_br, color.r, color.g, color.b);
  ...
  function getProjectData() {
    ...
    return {
      ...
      settings: { freezeTapToMove: !!freezeTapToMove, markerMode: !!markerMode },
      markers,
      terrain: { tiles },
      height
    };
  ...
  function applyProjectData(data) {
    ...
    if (data.height && Array.isArray(data.height.field)) {
      if (data.height.field.length === heightTool.heights.length) {
        ...
        if (Array.isArray(data.height.pins)) heightTool.pinned = new Set(data.height.pins);
  ...
  function addFreezeToggle() {
    ...
    style.textContent = `
      .hud-freeze{position:fixed;top:12px;left:12px;z-index:20;display:flex;align-items:center;gap:8px;background:rgba(30,32,37,0.85);color:#e8e8ea;padding:8px 10px;border:1px solid rgba(255,255,255,0.1);border-radius:6px;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);font:600 12px/1.2 -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Inter,sans-serif}
      .switch{position:relative;display:inline-block;width:44px;height:24px}.switch input{opacity:0;width:0;height:0}
      .slider{position:absolute;cursor:pointer;inset:0;background:#3a3d46;transition:.2s;border-radius:999px;box-shadow:inset 0 0 0 1px rgba(255,255,255,.1)}
      ...
      input:checked + .slider{background:#00aaff}input:checked + .slider:before{transform:translateX(20px)}
      input:disabled + .slider{filter:grayscale(.3);opacity:.65;cursor:not-allowed}
    `;
    ...
    hud.innerHTML = `
      <label class="switch" title="Freeze tap-to-move">
        <input type="checkbox" id="freezeMoveToggle"><span class="slider"></span>
      </label><span>Freeze tap-to-move</span>`;
    document.body.appendChild(hud);

    freezeCheckboxEl = hud.querySelector('#freezeMoveToggle');
    freezeCheckboxEl.addEventListener('change', () => {
      if (markerMode || paintingMode || heightMode) { freezeCheckboxEl.checked = true; return; }
